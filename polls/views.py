...
class IndexView(generic.ListView):    #Class-Based View
    ...
    def get_queryset(self):
        ...
        filter_dtfrom = datetime.strptime(self.request.GET.get('filter_datefrom', timezone.now().replace(year=2022)),"%d/%m/%Y %-I:%M %p")
        filter_datefrom = datetime.strptime(filter.dtfrom ,"%Y-%m-%d %H:%M")  #"%d/%m/%Y %H:%M %p"
        filter_dtto = datetime.strptime(self.request.GET.get('filter_dateto', timezone.now()),"%d/%m/%Y %-I:%M %p")
        filter_dateto = datetime.strptime(filter_dtto, "%Y-%m-%d %H:%M") # timezone.now().strftime("%Y-%m-%d %H:%M")
